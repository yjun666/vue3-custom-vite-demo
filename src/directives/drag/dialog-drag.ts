export const dialogDrag = () => {
    // 实现方法相对简单， 容易控制边界
    const dialogdragFun = (el: HTMLElement, binding: any) => {
        // 通过拖拽header进行拖拽
        const dialogHeaderEl = binding.value.dragHand ? el.querySelector(binding.value.dragHand) : el.querySelector('.el-dialog__header');
        ...
        // 设置动态传参过来的style样式， 设置在.el-dialog上
        for ([key, value] of Object.entries(binding.value.style || {})) {
            dragDom.style[key] = value;
        }
    ...
    return {
        updated(el: HTMLElement, params: any) {
            setTimeout(() => {
                ...
                const dragDom: HTMLElement = (el.querySelector('.el-dialog') || el.querySelector('.ele-form-dialog') as HTMLElement);
